...
class Blog extends Component {
			...
			fetch(
				...
				.then((value) => {
					...
					for (let i = 0; i < value.items.length; i++) {
						...
						let fullDate = `${dayNames[date.getDay()]}, ${monthNames[
							date.getMonth()
	...
	render() {
		...
		if (this.state.showLeft) {
			left = (
				<div onClick={this.prev} className={`${classes.pagi} ${classes.pagileft}`}>
					<span />
					Newer
				</div>
			);
					...
					<div className={classes.nav}>
							...
							>
								<button
									...
									className={classes.box}
								>
									<ColorBox color={'#508991'} icon={'far fa-address-card'} title={'About'} />
								</button>
							</ScrollAnimation>
						<ScrollAnimation
						...
						>
							<button
								...
								className={classes.box}
							>
								<ColorBox color={'#004346'} icon={'fas fa-code'} title={'Skills'} />
							</button>
						</ScrollAnimation>

						<ScrollAnimation animateOnce={true} duration={2} animateIn="fadeIn" animateOut="fadeOutUp">
						...
						>
							<button
								...
								className={classes.box}
							>
								<ColorBox color={'#09BC8A'} icon={'far fa-images'} title={'Portfolio'} />
							</button>
						</ScrollAnimation>
						...
						>
							<button
								...
								className={classes.box}
							>
								<ColorBox color={'#1c486b'} icon={'far fa-id-card'} title={'Contact'} />
							</button>
						</ScrollAnimation>
					</div>
				</div>
						...
						return (
							<div className={classes.post} key={index}>
								<div className={classes.darker}>
									<a href={`${post.url}`} target="_blank" className={classes.readmore}>
										Read More
									</a>
								</div>
								<img
